...
Sub ExportToPDFAndEmailMulttrans()
    Dim ws As Worksheet
    ...
    Set ws = ThisWorkbook.Sheets("Quo (Mult) Trans")
...
Sub RecordQuoteDetails(ByVal pdfFileName As String)
    ...
    Set quoSheet = ThisWorkbook.Sheets("Quo (Mult) Trans")
    Set salesSheet = ThisWorkbook.Sheets("Sales Tbls - Mult Prod")
    ...
    sales = salesSheet.Range("E8").Value
    costOfSales = salesSheet.Range("E10").Value
    grossProfit = salesSheet.Range("E21").Value
...
Sub send_email_signature(ByVal pdfFileName As String)
    ...
    ' Set reference to the worksheet
    Set sh = ThisWorkbook.Sheets("Quo (Mult) Trans")
